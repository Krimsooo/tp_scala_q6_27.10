...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] =  (x,l) match{
    ...
    case (x, head :: tail) if x<0 => None
    case (x, head:: tail) => nthA(x-1,tail )
  }
  ...
  def salarySum(employees: List[Employee]): Double = {
    def sum(employees : List[Employee], acc:Double): Double = employees match {
      case Nil => acc
      case head :: tail => sum(tail,acc+head.salary)
    }
    sum(employees,0.0)
  }
  ...
  def addressOf(users: List[User]): List[String] = {
    def adress(users : List[User], l: List[String]): List[String] = users match {
      case Nil => l
      case head :: tail => adress(tail,l.appended(head.address))
    }
    adress(users,List())
  }
